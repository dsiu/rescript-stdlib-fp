...
function initialize(length, f) {
  return $$Array.fromInitializer(length, index => TableclothChar.toString(f(index))).join("");
}
...
function fromList(t) {
  return List.toArray(t).map(character => String.fromCharCode(TableclothChar.toCode(character))).join("");
}
...
function fold(t, initial, f) {
  return $$Array.reduce(toArray(t), initial, (a, ch) => f(a, ch));
}

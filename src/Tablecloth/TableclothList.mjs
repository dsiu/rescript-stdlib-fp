...
function sum(t, M) {
  return Core__List.reduce(t, M.zero, M.add);
}
...
function initial(l) {
  let match = Core__List.reverse(l);
  if (match) {
...
function fold(t, initial, f) {
  return Core__List.reduce(t, initial, (a, b) => f(a, b));
}
...
function removeAt(t, index) {
  ...
  let match = Core__List.splitAt(t, index);
  if (match === undefined) {
    return t;
  }
  let t$1 = Core__List.tail(match[1]);
  if (t$1 !== undefined) {
...
function initialize(length, f) {
  return Core__List.fromInitializer(length, a => f(a));
}
...
let take = Core__List.take;

let drop = Core__List.drop;

let all = Core__List.every;

let find = Core__List.find;

let includes = Core__List.has;

let append = Core__List.concat;

...
import * as TableclothOption from "./TableclothOption.res.mjs";
...
function fromList(comparator, l) {
  let values = Stdlib_List.toArray(l);
  return Belt_Map.fromArray(values, Internal.toBeltComparator(comparator));
}
...
function keys(m) {
  return Stdlib_List.fromArray(Belt_Map.keysToArray(m));
}

function values(m) {
  return Stdlib_List.fromArray(Belt_Map.valuesToArray(m));
}
...
function fromList$1(l) {
  return fromArray$1(Stdlib_List.toArray(l));
}

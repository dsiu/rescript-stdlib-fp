...
let initialize = $$Array.fromInitializer;

function range(fromOpt, to_) {
  let from = fromOpt !== undefined ? fromOpt : 0;
  return $$Array.fromInitializer(to_ - from | 0, i => i + from | 0);
}
...
function fold(t, initial, f) {
  return $$Array.reduce(t, initial, (a, b) => f(a, b));
}

function foldRight(t, initial, f) {
  return $$Array.reduceRight(t, initial, (a, b) => f(a, b));
}
...
function map3(as_, bs, cs, f) {
  let minLength = $$Array.reduce([
    bs.length,
    cs.length
  ], as_.length, Primitive_int.min);
  return $$Array.fromInitializer(minLength, i => f(Option.getExn(as_[i], undefined), Option.getExn(bs[i], undefined), Option.getExn(cs[i], undefined)));
}
...
function sliding(stepOpt, a, size) {
  ...
  if (size > n) {
    ...
    return $$Array.fromInitializer(1 + Primitive_int.div(n - size | 0, step) | 0, i => $$Array.fromInitializer(size, j => Option.getExn(a[Math.imul(i, step) + j | 0], undefined)));
  }
}
...
function intersperse(t, sep) {
  return $$Array.fromInitializer(Primitive_int.max(0, (t.length << 1) - 1 | 0), i => {
    if (i % 2 !== 0) {
      return sep;
    } else {
...
function partition(t, f) {
  ...
  return [
    List.toArray(match[0]),
    List.toArray(match[1])
  ];
}
...
function unzip(t) {
  return [
    $$Array.fromInitializer(t.length, i => Option.getExn(t[i], undefined)[0]),
    $$Array.fromInitializer(t.length, i => Option.getExn(t[i], undefined)[1])
  ];
}

function repeat(element, length) {
  return $$Array.fromInitializer(Primitive_int.max(length, 0), param => element);
}

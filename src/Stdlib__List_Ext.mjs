...
function foldLeft(xs, f) {
  let init = Core__List.getExn(xs, 0);
  let rest = Core__List.tailExn(xs);
...
function transpose(_a) {
  while (true) {
    ...
    let match = a.hd;
    if (match) {
      let match$1 = Core__List.unzip(Core__List.map(a.tl, y => [
...
function combinationIf2(a, b, f) {
  return Core__List.reduce(a, /* [] */0, (acc, x) => Core__List.concat(acc, Core__List.reduce(b, /* [] */0, (acc, y) => {
    let r = f(x, y);
    if (r !== undefined) {

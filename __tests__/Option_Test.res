...
describe("map", () => {
  test("returns transformed value from inside option arg", () =>
    expect(Some(9)->Option.map(x => x * x))->toEqual(Some(81))
  )
...
describe("map2", () => {
  test("returns transformed value from two option arg", () =>
    expect(Option.map2(Some(3), Some(4), Int.add))->toEqual(Some(7))
  )

  test("returns none", () => expect(Option.map2(Some(3), None, Int.add))->toEqual(None))
  test("returns none", () => expect(Option.map2(None, Some(4), Int.add))->toEqual(None))
})

describe("andThen", () => {
  test("returns result of callback", () =>
    expect(Option.andThen(Some(list{1, 2, 3}), List.head))->toEqual(Some(1))
  )

  test("returns none", () => expect(Option.andThen(Some(list{}), List.head))->toEqual(None))
})

describe("unwrap", () => {
  test("returns unwrapped [option('a)]", () => expect(Option.unwrap(Some(42), 99))->toEqual(42))

  test("returns default", () => expect(Option.unwrap(None, 99))->toEqual(99))
})
